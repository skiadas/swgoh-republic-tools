...
from swgohdata import DEFAULT_COMLINK
        ...
        with SwgohComlink(url=DEFAULT_COMLINK) as comlink:
            for key, val in cache.items():
                if val is None:
                    ...
                    (raw_dir / f"{key}.json").write_text(json.dumps(cache[key], indent=1, ensure_ascii=False))
    return (
...
def main(argv=None):
    ...
    parser.add_argument("--outdir", type=Path, default=PROJECT / "data")
    ...
    tbd, camp, de, tables = fetch_raw(outdir, args.refresh)
